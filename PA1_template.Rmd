...
```
The mean number steps per day is `r round(meansteps, 2)`.  
...
```
The mean number steps per day with missing values imputed is `r round(meanstepsimp, 2)`.  

The median number steps per day with missing values imputed is `r medianstepsimp`.
